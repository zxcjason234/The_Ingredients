...
public class recipe_ingredient_rec_random extends AppCompatActivity
{
...
//        lv2=(ListView)findViewById(R.id.list2);
        ...
        DISHES_TYPE = intent.getStringExtra("dt");
        FOOD_MAIN= intent.getStringExtra("fm");
        COUNTRY_CUSINE= intent.getStringExtra("cc");
        M_OR_V= intent.getStringExtra("mv");
        descr = intent.getStringExtra("dc");
        PROVENANCE= intent.getStringExtra("pro");
        STEP= intent.getStringExtra("ST");
        img_url   = intent.getStringExtra("i_p");
        ...
        dish_title = intent.getStringExtra("t");
        lack_ing_title= intent.getStringExtra("LIT");
        fid= intent.getStringExtra("fid");
//        Toast.makeText(getApplicationContext(),title,Toast.LENGTH_SHORT).show();
        url2 = apiurl+"?test="+dish_title;
//        Toast.makeText(getApplicationContext(),url2,Toast.LENGTH_SHORT).show();

        fetch_Fridge_data_into_array(url2,fid);


//        split_String_into_array(lack_ing_title);
        btn_back.setOnClickListener(view -> {
//            Intent i = new Intent(recipe_ingredient_rec_random.this, dishes_info_rec_random.class);
//            i.putExtra("t", dish_title);
//            i.putExtra("fid", fid);
//            i.putExtra("dt", DISHES_TYPE);
//            i.putExtra("fm", FOOD_MAIN);
//            i.putExtra("cc", COUNTRY_CUSINE);
//            i.putExtra("mv", M_OR_V);
//            i.putExtra("dc", descr);
//            i.putExtra("pro", PROVENANCE);
//            i.putExtra("ST", STEP);
//            i.putExtra("i_p", img_url);
//            i.putExtra("did", did);
...
//        }
//
//        recipe_ingredient_rec_random.myadapter_lacked lacked_adapter = new recipe_ingredient_rec_random.myadapter_lacked(getApplicationContext(), lack_ing_title_arr);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                Intent i = new Intent(recipe_ingredient_rec_random.this, dishes_info.class);
                i.putExtra("t", dish_title);
                i.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
                startActivity(i);
//                finish();
    ...
    public void fetch_Fridge_data_into_array(String s, String fid)
    {
        String ip="http://192.168.0.195/yu/fridge_ingredient.php?FridgeID="+fid;
        Log.i("ip",ip);
        ...
        class  dbManager extends AsyncTask<String, Void, String>
        {
            protected void onPostExecute(String data)
            {
                try {
                    ...
                    fetch_data_into_array(url2,Ingredient_Title);


//                    recipe_ingredient_rec_random.myadapter adptr = new recipe_ingredient_rec_random.myadapter(getApplicationContext(), Ingredient_title, Quantity, State_and_Unit);
...
//                    Toast.makeText(getApplicationContext(),Ingredient_title[0]+Quantity[0] +State_and_Unit[0]+ing_row[0], Toast.LENGTH_LONG).show();
                    ...
                    for (String a: result_title)
                    {
                        System.out.println(a);
                    }


                    String[] arr = new String[result_title.size()];
                    arr = result_title.toArray(arr);
                    for (String a: arr)
                    {
                        System.out.println(a);
                    }
                    ...
                    myadapter_lacked lacked_adapter = new myadapter_lacked(getApplicationContext(), arr);
                    lv2.setAdapter(lacked_adapter);
                    setListViewHeight(lv2);
                    lacked_adapter.notifyDataSetChanged();
...
//                    if(str_count == "")
//                    {
//                        Snackbar.make(v,"請輸入數量!", Snackbar.LENGTH_LONG).setAction("Action", null).show();
//                    }
//                    else if(count < 0){
...
//                                data[1] = str_count;
//                                data[2] = ttl[position];
//                                PutData putData = new PutData("http:///yu/add_to_fridge_purchasing_list.php", "POST", field, data);
//                                if (putData.startPut()) {
...
////                                            Toast.makeText(getApplicationContext(),result, Toast.LENGTH_SHORT).show();
//                                            Log.i("message: ", result);
//                                            Snackbar.make(v,"以新增 食材名稱: "+ttl[position]+" 數量: "+str_count1+" 到採購清單", Snackbar.LENGTH_LONG).setAction("Action", null).show();
//                                            Log.i("INSERT sucess: ", "以新增 食材名稱: "+ttl[position]+" 數量: "+str_count1+" 到採購清單");
//                                        }
//                                        else if (result.equals("UPDAYTE sucess")){
////                                            Toast.makeText(getApplicationContext(),result, Toast.LENGTH_SHORT).show();
//                                            Log.i("message: ", result);
//                                            Snackbar.make(v,"以把 食材名稱: "+ttl[position]+" 在採購清單的數量更新成: "+str_count1, Snackbar.LENGTH_LONG).setAction("Action", null).show();
//                                            Log.i("UPDAYTE sucess: ", "以把 食材名稱: "+ttl[position]+" 在採購清單的數量更新成: "+str_count1);
//                                        }
//                                        else {
//                                            Log.i("error: ", result);
//                                            Snackbar.make(v, result, Snackbar.LENGTH_LONG).setAction("Action", null).show();
